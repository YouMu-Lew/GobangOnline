...
export default class Board extends Component {
    ...
    @property(Node)
    GGmenu: Node | null = null;      //  结算界面
    @property(Node)
    whiteWin: Node | null = null;      //  white win
    @property(Node)
    blackWin: Node | null = null;      //  black win
    @property(Node)
    drawNode: Node | null = null;      //  draw 和棋
    @property(Prefab)
    touchDotPrefab: Prefab | null = null;   // 触摸点预制体
    @property(Prefab)
    ...
    onLoad() {
        ...
        this.blockWidth = (this.node.getComponent(cc.UITransform).width / (GM.vLines - 1));     // 每块区域的宽度
        this.blockHeight = (this.node.getComponent(cc.UITransform).height / (GM.hLines - 1));   // 每块区域的高度
    ...
    putPiece(x: number, y: number) {
        ...
        // 是否棋盘已无处落子
        if (GM.getNumOfPiece() == GM.hLines * GM.vLines) {
