import { Component, Label, _decorator, director, Node } from "cc";
import { FightRoomState, global } from "../hw-gobe/script/hw_gobe_global_data";
import { GobeFightReady } from "../hw-gobe/script/gobe_fight_ready";
import { GobeFight } from "../hw-gobe/script/gobe_fight";
import { GameSceneType } from "../hw-gobe/script/frame_sync";
import { sleep } from "../hw-gobe/script/gobe_util";
import { GOBEError } from "../cs-huawei/hwgobe/GOBE/GOBE";

const { ccclass, property } = _decorator;

@ccclass('PvPGame')
export class PvPGame extends Component {
                ...
                if (global.room.customRoomProperties && global.room.customRoomProperties.length > 0) {
                    try {
                        ...
                        if (roomProp.curFrameId) {
                            console.log("重置房间帧同步起始帧id:" + roomProp.curFrameId);
                            global.room.resetRoomFrameId(roomProp.curFrameId);
                        }
                    } catch (error) {
                        console.error(error);
                        console.log("重置房间帧同步起始帧id:0");
    ...
    */
        ...
        global.room.update()
            .then((room) => {
                ...
                if (playerInfo.playerId === global.playerId) {
                    console.log("玩家重连加入 id:" + playerInfo.playerId);
                    //自己是否还在房间
                ...
                if (this._curRoomState == FightRoomState.Ready && room.isSyncing == true) {
                    ...
                    console.log("当前房间已经结束，即将离开房间");
                    this.scheduleOnce(() => {
                        global.client.leaveRoom()
                            .then(() => {
                                console.log("onJoin room.isSyncing == false leaveRoom success");
                            });
                        director.loadScene("MainMenu");
                    }, 2)
                }
            }).catch((e) => {
                console.log("onJoin global.room.update err: ", e);
                this.scheduleOnce(() => {
                    global.client.leaveRoom()
                        .then(() => {
                            console.log("onJoin update error leaveRoom success");
                        });
                    director.loadScene("MainMenu");
                }, 2)
            });
                ...
                if (!global.isConnected) {
                    global.room.reconnect()
                        ...
                        .catch((error: GOBEError) => {
                            console.log("reconnect err,", error);
                            //房间不存在
                            if (error.code == 101117) {
                                console.log("房间不存在，离开房间");
                                director.loadScene("MainMenu");
                                return;
                            }
                            //是否达到最大重连次数
    ...
    onRecvFromServer(data: GOBE.RecvFromServerInfo) {
        console.log('onReceiveFromServer:' + JSON.stringify(data));
        //解析数据
        let res = JSON.parse(data.msg);
        if (res.type === "GameEnd") {
            console.log("游戏结束 res.result:" + res.result);
